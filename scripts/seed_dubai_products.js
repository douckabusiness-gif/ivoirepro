...
async function seed() {
  ...
  // Find suitable category IDs or fallback
  const beautyCat = categories.find(c => c.id === 'cat-beauty' || c.slug.includes('beaute') || c.slug.includes('parfum')) || categories[0];
  const watchCat = categories.find(c => c.id === 'cat-watches' || c.slug.includes('montre') || c.slug.includes('bijou')) || categories[0];
  const fashionCat = categories.find(c => c.id === 'cat-fashion' || c.slug.includes('mode') || c.slug.includes('habill')) || categories[0];
  const phoneCat = categories.find(c => c.id === 'cat-phones' || c.slug.includes('phone') || c.slug.includes('telephon')) || categories[0];
  ...
  for (const prod of dubaiProducts) {
    ...
    console.log('Upserted successfully:', prod.title);
  }

  console.log('✅ ALL 8 DUBAI LUXURY PRODUCTS SEEDED SUCCESSFULLY!');
}
